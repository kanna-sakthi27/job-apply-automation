# foreign-rotation — auto-apply run (Command Code)
...
---
Foreign direct-apply run — ROTATION country block (evening). Follow foreign-jobs-automation/RULESET.md exactly (read it + ATS_PLAYBOOK.md + COMPANY_TARGETS.md + ATS_REGISTRY.md + foreign-jobs-automation/run-log.md + linkedin-automation/answers.md (shared answer bank) first). Rotate the country: Australia, then UK, then Ireland, then Germany/Netherlands, then UAE, then remote-global employers (cycle per RULESET §3; pick the next country after whatever ran previously per ATS_REGISTRY.md/run log). Drive the candidate's real Brave with the agent-browser CLI (see the header; `agent-browser connect 9222`).
...
LANGUAGE & COUNTRY (candidate policy 2026-09-07): the candidate only speaks English. Apply ONLY to postings in English where English is the working language. Skip any role requiring Finnish/German/French/Dutch/Swedish/etc. - even 'bilingual nice-to-have' is a wall unless the posting clearly works in English. This rotation run: target UK, then FINLAND (Wolt, Supercell, Nokia, KONE, Smartly, Unity - Helsinki tech is English-friendly), then Ireland, Netherlands, Germany/EU - using COMPANY_TARGETS.md's Europe block. Skip SC-clearance / 5-yr-UK-residency / settled-status. Only English postings.

## APPLICANT OVERRIDES 2026-09-07 (apply to every run)
1. HIT 30/DAY PER CHANNEL: daily target is 30 submitted applications per channel. Do NOT stop early while in-scope supply remains. Per-run attempt cap 15 (Foreign-Rot: attempt up to day cap). Keep rotating keywords/markets (US/UK/IE/EU-English/CA/SG/AU/remote) until 30/day is reached or the fresh pool is genuinely exhausted; then log a RUN SUMMARY with counts.
