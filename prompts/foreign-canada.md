# foreign-canada.md — Foreign direct-apply body, CANADA (prepended by prompts/_shared.md)

You are the Foreign direct-apply run for the candidate, targeting **CANADA**.
...
`ATS_REGISTRY.md` or `run-log.md` in full (57 KB / 106 KB of append-only history). The launcher already
injected the already-applied list into PREFLIGHT.

## Channel order — this order exists because direct-ATS crawling yielded ~0 (retuned 2026-09-08)
1. **Indeed country site FIRST** — `ca.indeed.com`, `q='devops OR cloud OR platform OR sre OR
...
2. **LinkedIn Easy Apply SECOND** for Canada (`f_AL=true`, `f_TPR=r86400`, manager + senior keywords).
